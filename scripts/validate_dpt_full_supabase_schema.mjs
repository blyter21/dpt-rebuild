...
if (process.env.DPT_VALIDATE_PRIVATE_IMPORT === '1') {
  ...
  const coreResult = await db.query(`
    select
      ...
      (select count(*)::int from public.tournament_qualifiers) as tournament_qualifiers,
      (select count(*)::int from public.toc_qualified_types) as toc_qualified_types,
      (select count(*)::int from public.toc_qualified_tournaments) as toc_qualified_tournaments,
...
const workflowSecurityResult = await db.query(`
  select
    ...
    not has_function_privilege('anon','public.dpt_admin_save_tournament_relationships(bigint,jsonb)','execute') as anon_cannot_save_tournament_relationships,
    has_function_privilege('authenticated','public.dpt_admin_save_tournament_relationships(bigint,jsonb)','execute') as authenticated_can_save_tournament_relationships,
...
if (coreImportCounts) {
  const expectedCoreCounts = {
    ...
    events: 82,
    tournaments: 271,
    tournament_qualifiers: 58,
    toc_qualified_types: 0,
    toc_qualified_tournaments: 0,
    blind_structures: 8,
