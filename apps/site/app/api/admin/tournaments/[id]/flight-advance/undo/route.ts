...
export async function POST(_request: Request, { params }: { params: { id: string } }) {
  ...
  const response = await dptAdminSupabaseFetch(context, '/rest/v1/rpc/dpt_admin_undo_flight_advancement', {
    method: 'POST', body: JSON.stringify({ p_flight_tournament_id: tournamentId }),
  });
  if (!response.ok) return NextResponse.json({ error: 'Unable to undo flight advancement', detail: await response.text() }, { status: response.status });
